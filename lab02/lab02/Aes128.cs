...
namespace Aes128
{
    public class Aes128Implementation
    {
        ...
        private byte[] InverseSBox => new byte[]
        {
        ...
        byte Multiply(byte value) => (byte)((value << 1) ^ ((value >> 7) & 0x1b));

        byte[] RoundConstant => new byte[] { 0x01, 0x02, 0x04, 0x08, 0x10, 0x20, 0x40, 0x80, 0x1b, 0x36 };
        ...
        private static void AddLastRoundKey(byte[] outState, byte[] k)
        {
            for (int i = 16 - 1; i >= 0; i--)
                outState[i] = (byte)(outState[i] ^ k[i]);
        }
